...
open class Node: Codable {
  ...
  public func getStyle<T: Style>(_ style: T.Type) -> T.StyleValueType? {
  ...
  public func setStyle<T: Style>(_ style: T.Type, _ value: T.StyleValueType?) throws {
    try validateStyleOrThrow(style)
    try getWritable().styles[style.name] = value
  }

  public func getStyles() -> StylesDict {
    return getLatest().styles
  }

  public func setStyles(_ stylesDict: StylesDict) throws {
    // TODO: validate all!
    try getWritable().styles = stylesDict
  }

  public func toggleStyle<T: Style>(_ style: T.Type) throws where T.StyleValueType == Bool {
    let currentValue = getStyle(style) ?? false
    try setStyle(style, !currentValue)
  }

  public func validateStyle<T: Style>(_ style: T.Type, value: T.StyleValueType?) -> T.StyleValueType? {
    do {
      try validateStyleOrThrow(style)
    } catch {
      return nil
