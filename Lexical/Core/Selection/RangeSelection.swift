...
public class RangeSelection: BaseSelection {
  ...
  internal func insertParagraph() throws {
    ...
    let anchorOffset = anchor.offset
    var currentElement: ElementNode? = nil
    ...
    if anchor.type == .text {
      ...
      let isInline = currentElement?.isInline() ?? false
      let textContentLength = isInline ? currentElement?.getTextContentSize() ?? 0 : anchorNode.getTextContentSize()
      ...
      guard let currentElement = try anchor.getNode() as? ElementNode else { return }
    ...
    guard let currentElement else {
      // should not happen, as currentElement is initialised in both branches of the above 'if'.
      // Not refactoring further at this time in order to keep the code flow the same as Lexical JS.
      getActiveEditor()?.log(.editor, .error, "unexpected lack of currentElement")
      return
    }
